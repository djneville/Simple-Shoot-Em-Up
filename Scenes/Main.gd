...
func _clamp_viewport() -> void:
	var player_entity: PlayerEntity = $CharacterBody2D
	...
	player_entity.global_position.x = clamp(
		player_entity.global_position.x, texture_width / 2, viewport_size.x - texture_width / 2
	)

	player_entity.global_position.y = clamp(
		player_entity.global_position.y, texture_height / 2, viewport_size.y - texture_height / 2
	)
