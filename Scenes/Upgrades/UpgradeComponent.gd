...
func _update_upgrade() -> void:
    ...
    var upgrade: UpgradeData = UpgradeManager.upgrades[self.current_upgrade_index]
    self.active_plane_sprite = Sprite2D.new()
    self.active_plane_sprite.set_texture(upgrade.plane_sprite_texture)
    self.active_collision_shape = CollisionShape2D.new()
    self.active_collision_shape.set_shape(upgrade.collision_shape)
    self.active_speed = upgrade.speed
    self.active_projectile_type = upgrade.projectile_type
