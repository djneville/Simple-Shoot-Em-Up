...
@export var fracturing_coefficient: float = 0.5
...
var hydrofracture_controller: HydrofractureController = HydrofractureController.new()
...
func setup_glacier() -> void:
    ...
    glacier_mass_distribution.initialize_mass_distribution(WIDTH, HEIGHT, glacier_map)
    hydrofracture_controller.initialize(max_fracture_depth, fracturing_coefficient)
    iceberg_manager.min_mass = min_iceberg_mass
    iceberg_manager.max_icebergs = max_active_icebergs
...
func _on_iceberg_created(_position: Vector2i) -> void:
    ...
    print("PARTICLES CREATED AT: ", particles_instance.position)
...
func _on_hydrofracture_cycle() -> void:
    ...
    for fracture_start_coordinates: Vector2i in selected_fracture_starts:
        hydrofracture_controller.propagate_fracture(glacier_mass_distribution, glacier_map, fracture_start_coordinates)
